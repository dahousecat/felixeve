...
let menu = {
    ...
    facade: document.querySelector('.facade'),
    body: document.querySelector('body'),
    topBar: document.getElementById('top-bar-anim'),
    middleBar: document.getElementById('middle-bar-anim'),
    bottomBar: document.getElementById('bottom-bar-anim'),
    init: function() {
        menu.body.classList.add('js');
        menu.button.addEventListener('click', menu.buttonClick);
        document.querySelectorAll('.menu__link').forEach(item => {
            item.addEventListener('click', menu.linkClick)
        })
    },
    buttonClick: function() {
        menu.addAnimation();
        setTimeout(menu.openClose, 0);
        setTimeout(menu.removeAnimation, 400);
    },
    openClose: function () {
        const isOpen = menu.menu.classList.contains('menu--open');
        menu.button.setAttribute('aria-expanded', !isOpen);
        menu.menu.classList.toggle('menu--open');
        menu.body.classList.toggle('lock');
        menu.facade.classList.toggle('facade--show');
        menu.animateBars();
        if (isOpen) {
            menu.menu.querySelector('a').focus();
        }
    },
    addAnimation() {
        menu.menu.classList.add('menu--animate');
    },
    removeAnimation: function() {
        menu.menu.classList.remove('menu--animate');
    },
    animateBars: function() {
        menu.reverse(menu.topBar).beginElement();
        menu.reverse(menu.middleBar).beginElement();
        menu.reverse(menu.bottomBar).beginElement();
    },
    reverse: function(element) {
        const fromVal = element.getAttribute('from');
        const toVal = element.getAttribute('to');
        element.setAttribute('from', toVal);
        element.setAttribute('to', fromVal);
        return element;
    },
    linkClick: function() {
        menu.openClose();
...
//
//     const topBar = document.getElementById('top-bar-anim');
//     const middleBar = document.getElementById('middle-bar-anim');
//     const bottomBar = document.getElementById('bottom-bar-anim');
//
//     if (menu_button.dataset.state !== 'initial') {
//         reverse(topBar)
//         reverse(middleBar)
//         reverse(bottomBar)
//     }
//
//     if (menu_button.dataset.state === 'closed' || menu_button.dataset.state === 'initial') {
...
//
//     topBar.beginElement();
//     middleBar.beginElement();
//     bottomBar.beginElement();
// });
//
...
galite('create', 'UA-36080651-1', 'auto');
